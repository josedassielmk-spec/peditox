...
public class PeditoWhistleItem extends Item {
    ...
    public InteractionResult use(Level level, Player player, InteractionHand usedHand) {
        ...
        // Activarse agachándose con el silbato en la mano
        if (player.isShiftKeyDown() || player.isCrouching()) {
            if (!level.isClientSide()) {
                ...
                // Sonido de silbato usando NOTE_BLOCK_FLUTE a tono alto
                level.playSound(null, player.getX(), player.getY(), player.getZ(),
                        SoundEvents.NOTE_BLOCK_FLUTE.value(), SoundSource.PLAYERS, 1.5F, 1.9F);
