...
public class PeditoSonicBoomGoal extends Goal {
    ...
    public void tick() {
        ...
        if (this.attackTimer > 30) {
            ...
            double targetX = dirToTarget.x * Math.cos(angleOffset) - dirToTarget.z * Math.sin(angleOffset);
            double targetZ = dirToTarget.x * Math.sin(angleOffset) + dirToTarget.z * Math.cos(angleOffset);
            Vec3 spreadDir = new Vec3(targetX, dirToTarget.y, targetZ).normalize();
            
            Vec3 optimalPos = target.position().subtract(spreadDir.scale(1.8)).add(0, 0.5, 0);
                
            this.pedito.getMoveControl().setWantedPosition(optimalPos.x, optimalPos.y, optimalPos.z, 1.8D);
        ...
        if (this.pedito.level() instanceof ServerLevel serverLevel) {
            if (this.attackTimer > 0) {
                ...
                Vec3 offset = new Vec3((this.pedito.getRandom().nextDouble() - 0.5) * 4.0, (this.pedito.getRandom().nextDouble() - 0.5) * 4.0, (this.pedito.getRandom().nextDouble() - 0.5) * 4.0);
                Vec3 spawnPos = peditoPos.add(offset);
                Vec3 velocity = peditoPos.subtract(spawnPos).scale(0.1);
                
                serverLevel.sendParticles(ParticleTypes.CLOUD, spawnPos.x, spawnPos.y, spawnPos.z, 1, velocity.x, velocity.y, velocity.z, 0.0);
            } else if (this.attackTimer == 0) {
                // Fire custom Fart Wave (instead of Warden Sonic Boom)
                this.pedito.playSound(ModSounds.PEDITO_SONIC_BOOM, 0.7F, 1.0F);
                
                Vec3 startPos = this.pedito.position().add(0, this.pedito.getBbHeight() / 2.0, 0);
                ...
                for (int i = 0; i < 20; i += 2) {
                    ...
                    for (int j = 0; j < 8; j += 2) {
                        ...
                        Vec3 ringPos = particlePos.add(right.scale(Math.cos(angle) * radius)).add(actualUp.scale(Math.sin(angle) * radius));
                        serverLevel.sendParticles(ParticleTypes.CAMPFIRE_COSY_SMOKE, ringPos.x, ringPos.y, ringPos.z, 1, 0, 0, 0, 0);
                        serverLevel.sendParticles(ParticleTypes.CLOUD, ringPos.x, ringPos.y, ringPos.z, 1, 0, 0, 0, 0);
                    }
                }
                ...
                if (myIndex == 0 || this.pedito.getRandom().nextBoolean()) {
                    target.hurtServer(serverLevel, this.pedito.level().damageSources().mobAttack(this.pedito), 8.0F);
