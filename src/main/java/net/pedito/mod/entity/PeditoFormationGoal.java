...
public class PeditoFormationGoal extends Goal {
    ...
    private static final java.util.Map<java.util.UUID, Integer> PLAYER_FORMATIONS = new java.util.concurrent.ConcurrentHashMap<>();
        ...
        if (formation == FormationType.DEFAULT) {
            ...
            switch (formation) {
                ...
                case V_SHAPE: {
                    ...
                    double offsetX = backX * (row * 1.3 + 2.0) + rightX * (side * row * 1.3);
                    double offsetZ = backZ * (row * 1.3 + 2.0) + rightZ * (side * row * 1.3);
                    double offsetY = 1.2 + Math.sin(time * 1.5 + myIndex * 0.3) * 0.15;
                ...
                case CIRCLE: {
                    ...
                    double angle = baseAngle + (myIndex * (2 * Math.PI / total)) + (time * 0.3); // Rotación lenta
            ...
            this.pedito.getLookControl().setLookAt(lookX, this.pedito.getEyeY(), lookZ, 10.0F, 10.0F);
        }
