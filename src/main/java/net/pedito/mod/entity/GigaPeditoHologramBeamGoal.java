...
public class GigaPeditoHologramBeamGoal extends Goal {
  ...
  public boolean canUse() {
    ...
    if (
      target == null || !target.isAlive() || target.getMaxHealth() <= 100
    ) return false;
    List<PeditoEntity> allies = this.pedito
      ...
      .getEntitiesOfClass(
        ...
        e ->
          ...
          (e.getSquadRole() == PeditoEntity.SquadRole.TACTICAL ||
            e.getSquadRole() == PeditoEntity.SquadRole.SOLO)
      );
  ...
  public boolean canContinueToUse() {
    return (
      this.phase < 4 &&
      this.pedito.getTarget() != null &&
      this.pedito.getTarget().isAlive()
    );
  }
  ...
  public void start() {
    this.phase = 0;
    this.timer = 30;
    /* Phase 1: 30 ticks */ this.pedito.playSound(
      ModSounds.PEDITO_SUMMON,
      1.0F,
      0.75F +
        (this.pedito.getRandom().nextFloat() -
          this.pedito.getRandom().nextFloat()) *
          0.02F
    );
  }
  ...
  public void tick() {
    ...
    if (myIndex < 10) {
      /* Form a 6.0 diameter circle (radius 3.0), 12.0 blocks above target */ this.centerX =
        target.getX();
      ...
      this.pedito
        .getMoveControl()
        .setWantedPosition(this.targetX, this.targetY, this.targetZ, 2.5D);
      this.pedito
        .getLookControl()
        .setLookAt(this.centerX, this.centerY, this.centerZ, 30.0F, 30.0F);
      if (this.pedito.level() instanceof ServerLevel serverLevel) {
        /* Gold lines to center */ serverLevel.sendParticles(
          new DustParticleOptions(0xFFD800, 0.4F),
          this.pedito.getX(),
          this.pedito.getY(),
          this.pedito.getZ(),
          1,
          (this.centerX - this.pedito.getX()) * 0.5,
          (this.centerY - this.pedito.getY()) * 0.5,
          (this.centerZ - this.pedito.getZ()) * 0.5,
          0.5
        );
        /* Only leader does the big stuff */ if (myIndex == 0) {
          /* Hologram head (abstract representation with particles) */ for (
            int i = 0;
            i < 20;
            i++
          ) {
            double px =
              this.centerX + (this.pedito.getRandom().nextDouble() - 0.5) * 4.0;
            double py =
              this.centerY + (this.pedito.getRandom().nextDouble() - 0.5) * 4.0;
            double pz =
              this.centerZ + (this.pedito.getRandom().nextDouble() - 0.5) * 4.0;
          ...
          if (this.phase == 1) {
            ...
            target.addEffect(
              new MobEffectInstance(MobEffects.SLOWNESS, 20, 3, false, false)
            );
            if (this.timer % 4 == 0) {
            ...
            /* Beam particles */ for (
              ...
              serverLevel.sendParticles(
                new DustParticleOptions(
                  this.pedito.getRandom().nextInt(0xFFFFFF),
                  1.0F
                ),
                this.centerX,
                y,
            ...
            /* Impact particles */ serverLevel.sendParticles(
    ...
    if (this.phase == 0) {
      ...
      if (
        this.pedito.level() instanceof ServerLevel serverLevel && myIndex == 0
      ) {
        serverLevel.sendParticles(
      ...
      if (this.timer <= 0) {
        this.phase = 4;
        this.nextAttackTick =
          this.pedito.tickCount + (this.pedito.hasCopperSynergy() ? 880 : 1000);
      }
    }
  }
}
