...
public class ColorFusionAttackGoal extends Goal {
  ...
  public void tick() {
    ...
    this.pedito
      .getMoveControl()
      .setWantedPosition(optimalPos.x, optimalPos.y, optimalPos.z, 2.0D); // Fast movement
        ...
        for (LivingEntity e : entities) {
          ...
          if (distance > 0) {
            e.setDeltaMovement(
              e
                .getDeltaMovement()
                .add((dx / distance) * 2.0, 0.8, (dz / distance) * 2.0)
            );
          }
        }

        this.nextAttackTick =
          this.pedito.tickCount + (this.pedito.hasCopperSynergy() ? 105 : 120); // 6 seconds cooldown
      }
    }
  }
