...
public class WildAlphaBreedingGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        List<PeditoEntity> targets = this.alpha.level().getEntitiesOfClass(
                PeditoEntity.class,
                this.alpha.getBoundingBox().inflate(16.0D),
                e -> e.isAlive() && !e.isBaby() && !e.isTamedByOwner() && e.getVariant() == PeditoEntity.VARIANT_NORMAL && e != this.alpha
        );
    ...
    public boolean canContinueToUse() {
        if (this.alpha.isTamedByOwner() || this.alpha.getTarget() != null) return false;
        if (this.partner == null || !this.partner.isAlive() || this.partner.isTamedByOwner()) return false;
        return this.breedingTimer > 0 && this.alpha.distanceToSqr(this.partner) < 256.0D;
    }
    ...
    public void tick() {
        ...
        if (distSqr > 2.25D) { // Aproximadamente 1.5 bloques
        ...
        if (distSqr <= 3.0D && this.alpha.level() instanceof ServerLevel serverLevel) {
            // Efecto visual romántico y alegre
            if (this.breedingTimer % 10 == 0) {
                serverLevel.sendParticles(ParticleTypes.HEART, 
                    this.alpha.getX(), this.alpha.getY() + 0.6, this.alpha.getZ(), 
                    1, 0.2, 0.2, 0.2, 0.0);
                serverLevel.sendParticles(ParticleTypes.HAPPY_VILLAGER, 
                    this.partner.getX(), this.partner.getY() + 0.5, this.partner.getZ(), 
                    1, 0.2, 0.2, 0.2, 0.0);
            }
    ...
    private void spawnNewGroup() {
        ...
        for (int i = 0; i < spawnCount; i++) {
            PeditoEntity offspring = new PeditoEntity(Pedito.PEDITO_ENTITY, serverLevel);
            double rx = this.alpha.getX() + (this.alpha.getRandom().nextDouble() - 0.5) * 2.0;
            double ry = this.alpha.getY() + (this.alpha.getRandom().nextDouble() - 0.5) * 0.5;
            double rz = this.alpha.getZ() + (this.alpha.getRandom().nextDouble() - 0.5) * 2.0;
        ...
        serverLevel.playSound(null, this.alpha.getX(), this.alpha.getY(), this.alpha.getZ(),
                ModSounds.PEDITO_FART_SPAWN, SoundSource.NEUTRAL, 1.5F, 1.1F);
        serverLevel.sendParticles(ParticleTypes.CLOUD, this.alpha.getX(), this.alpha.getY() + 0.5, this.alpha.getZ(),
                15, 0.5, 0.3, 0.5, 0.05);
