...
public class PeditoAirstrikeGoal extends Goal {
  ...
  public boolean canUse() {
    ...
    List<LivingEntity> enemies = this.pedito
      ...
      .getEntitiesOfClass(
        ...
        e ->
          e.isAlive() &&
          e != this.pedito.getOwnerCustom() &&
          !(e instanceof PeditoEntity)
      );
  ...
  public void tick() {
    if (this.phase == 0) {
      ...
      this.pedito
        .getMoveControl()
        .setWantedPosition(this.targetX, this.targetY, this.targetZ, 2.0D);
      this.pedito
        .getLookControl()
        .setLookAt(this.strikeX, this.strikeY, this.strikeZ, 30.0F, 30.0F);
      ...
      this.pedito
        .getMoveControl()
        .setWantedPosition(this.targetX, this.strikeY, this.targetZ, 1.9D);
      ...
      if (this.pedito.level() instanceof ServerLevel serverLevel) {
        ...
        serverLevel.sendParticles(
          new DustParticleOptions(0xFF0000, 1.2F),
          this.pedito.getX(),
          this.pedito.getY() + 0.3,
          this.pedito.getZ(),
          6,
        ...
        if (this.pedito.getVariant() == PeditoEntity.VARIANT_RAINBOW) {
          int rgb =
            (this.pedito.getRandom().nextInt(256) << 16) |
            (this.pedito.getRandom().nextInt(256) << 8) |
            this.pedito.getRandom().nextInt(256);
          serverLevel.sendParticles(
          ...
          serverLevel.sendParticles(
            new DustParticleOptions(0x1A237E, 1.0F),
            this.pedito.getX(),
            this.pedito.getY() + 0.3,
            this.pedito.getZ(),
            4,
      ...
      boolean hitGround =
        this.pedito.getY() <= this.strikeY + 1.2 ||
        this.pedito.onGround() ||
        this.timer <= 0;
      if (hitGround) {
        // EXPLOSION!
        if (this.pedito.level() instanceof ServerLevel serverLevel) {
          ...
          for (LivingEntity enemy : enemies) {
            ...
            enemy.addEffect(
              new MobEffectInstance(MobEffects.POISON, 100, 1, false, false)
            );
            enemy.addEffect(
              new MobEffectInstance(MobEffects.SLOWNESS, 80, 2, false, false)
            );
            ...
            if (dist > 0) {
              enemy.setDeltaMovement(
                enemy
                  .getDeltaMovement()
                  .add((dx / dist) * 1.5, 0.6, (dz / dist) * 1.5)
              );
            }
          }
        }
      ...
      this.pedito
        .getLookControl()
        .setLookAt(this.targetX, this.strikeY, this.targetZ, 30.0F, 30.0F);
      ...
      if (this.pedito.level() instanceof ServerLevel serverLevel) {
        if (this.pedito.tickCount % 10 == 0) {
          ...
          AABB poolBox = new AABB(
            this.pedito.getX() - 1.5,
            this.strikeY - 1,
            this.pedito.getZ() - 1.5,
            this.pedito.getX() + 1.5,
            this.strikeY + 2,
            this.pedito.getZ() + 1.5
          );
          ...
          for (LivingEntity enemy : enemies) {
            enemy.addEffect(
              new MobEffectInstance(MobEffects.POISON, 80, 0, false, false)
            );
            enemy.addEffect(
              new MobEffectInstance(MobEffects.SLOWNESS, 60, 1, false, false)
            );
          }
        }
      }
      ...
      if (this.timer <= 0) {
        this.phase = 3;
        this.nextAttackTick =
          this.pedito.tickCount + (this.pedito.hasCopperSynergy() ? 440 : 500);
      }
    }
  }
}
