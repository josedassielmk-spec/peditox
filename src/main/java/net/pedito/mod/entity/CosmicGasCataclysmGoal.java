...
public class CosmicGasCataclysmGoal extends Goal {
  ...
  public boolean canUse() {
    ...
    /* Jefe con > 60% vida restante */ if (
      target.getMaxHealth() <= 100
    ) return false;
    /* Is boss */ if (
      target.getHealth() <= target.getMaxHealth() * 0.6f
    ) return false;
  ...
  public void start() {
    ...
    this.phase = 0;
    this.timer = 40;
    /* Phase 1: 40 ticks */ this.pedito.playSound(
  ...
  public void tick() {
    ...
    if (myIndex < 20) {
      if (this.phase < 4) {
        /* Only position in dome for first 4 phases */
        /* Dome positioning (Fibonacci sphere over a hemisphere) */
        double i = myIndex + 0.5;
        double phi = Math.acos(1.0 - (i / totalInDome));
        /* Only top half */
        double theta = Math.PI * (1.0 + Math.sqrt(5.0)) * i;
        double radius = 15.0;
        double tx = this.impactX + radius * Math.sin(phi) * Math.cos(theta);
        double ty = this.impactY + 20.0 + radius * Math.cos(phi) - 15.0;
        /* adjust to make it a dome over the player */ double tz =
          this.impactZ + radius * Math.sin(phi) * Math.sin(theta);
      ...
      if (
        ...
        /* Leader logic */ if (this.phase == 0) {
          /* Net lines (random connections) */ for (int n = 0; n < 5; n++) {
            PeditoEntity p1 = allies.get(
              this.pedito.getRandom().nextInt(totalInDome)
            );
            PeditoEntity p2 = allies.get(
              this.pedito.getRandom().nextInt(totalInDome)
            );
            serverLevel.sendParticles(
              new DustParticleOptions(0xCC00CC, 0.4F),
              p1.getX(),
              p1.getY(),
              p1.getZ(),
              1,
          ...
          this.meteorY -= 0.9;
          /* Descends at 18 m/s (0.9 blocks/tick) */ serverLevel.sendParticles(
          ...
          if (this.pedito.tickCount % 20 == 0) {
            ...
            for (LivingEntity enemy : enemies) {
              enemy.addEffect(
                new MobEffectInstance(MobEffects.POISON, 200, 2, false, false)
              );
              enemy.addEffect(
                new MobEffectInstance(MobEffects.WEAKNESS, 160, 2, false, false)
              );
              enemy.addEffect(
                new MobEffectInstance(MobEffects.SLOWNESS, 240, 2, false, false)
              );
            }
          }
        }
      ...
      if (this.phase == 4) {
        /* Exhaustion phase */ if (this.timer == 400 && myIndex == 0) {
          this.pedito.playSound(
        ...
        this.pedito.addEffect(
          new MobEffectInstance(MobEffects.SLOWNESS, 20, 2, false, false)
        ); /* 50% speed */
      }
    }
    this.timer--;
