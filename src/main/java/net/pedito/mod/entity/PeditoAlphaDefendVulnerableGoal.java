...
public class PeditoAlphaDefendVulnerableGoal extends TargetGoal {
    ...
    public boolean canUse() {
        ...
        if (owner.getHealth() < owner.getMaxHealth() * 0.5F) {
            ...
            if (ownerAttacker != null && ownerAttacker.isAlive() && this.canAttack(ownerAttacker, TargetingConditions.DEFAULT)) {
        ...
        List<PeditoEntity> allies = this.pedito.level().getEntitiesOfClass(
            ...
            e -> e.isAlive() && e.isTamedByOwner() && e.getOwnerCustom() == owner && e != this.pedito
        );

        for (PeditoEntity ally : allies) {
            if (ally.getHealth() < ally.getMaxHealth() * 0.5F) {
                LivingEntity allyAttacker = ally.getLastHurtByMob();
                if (allyAttacker != null && allyAttacker.isAlive() && this.canAttack(allyAttacker, TargetingConditions.DEFAULT)) {
