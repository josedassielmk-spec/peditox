...
public class PeditoGolemSummonGoal extends Goal {
  ...
  public boolean canUse() {
    ...
    List<PeditoGolemEntity> golems = this.pedito
      .level()
      .getEntitiesOfClass(
    ...
    if (!golems.isEmpty()) {
      this.nextAttackTick =
        this.pedito.tickCount + (this.pedito.hasCopperSynergy() ? 88 : 100); // Micro-cooldown por spam
      return false;
    }
  ...
  public void tick() {
    ...
    if (this.pedito.level() instanceof ServerLevel serverLevel) {
      ...
      // Spawn Golem
      if (this.castTimer == 0) {
        PeditoGolemEntity golem = Pedito.PEDITO_GOLEM_ENTITY.create(
          serverLevel,
        ...
        if (golem != null) {
          ...
          golem.addEffect(
            new MobEffectInstance(MobEffects.WITHER, 600, 0, false, false)
          );
        ...
        this.nextAttackTick =
          this.pedito.tickCount +
          (this.pedito.hasCopperSynergy() ? 1056 : 1200); // Cooldown 1200 ticks
      }
    }
  }
