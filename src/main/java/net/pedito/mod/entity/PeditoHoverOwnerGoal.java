  ...
  public void tick() {
    ...
    if (!this.pedito.isOwnerInDanger()) {
      ...
      this.pedito
        ...
        .setWantedPosition(
          owner.getX(),
          owner.getY() + owner.getBbHeight() / 2.0,
          owner.getZ(),
          1.5D
        );
      return;
    ...
    int peditosInThisLayer = Math.min(
      peditosInCurrentLayer,
      total - peditosBeforeCurrentLayer
    );
    ...
    double fraction =
      peditosInThisLayer <= 1
        ? 0.5
        : (double) indexInLayer / (peditosInThisLayer - 1);
    ...
    double offsetY =
      baseHeight + heightArc * 1.5 + Math.sin(time * 1.5 + myIndex) * 0.4;
    ...
    if (
      ...
      this.pedito
        .getMoveControl()
        .setWantedPosition(this.hoverX, this.hoverY, this.hoverZ, 1.4D);
    }
  }
}
