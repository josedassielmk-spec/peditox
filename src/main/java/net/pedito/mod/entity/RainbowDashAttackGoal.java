...
public class RainbowDashAttackGoal extends Goal {
  ...
  public void tick() {
    ...
    if (this.attackState == 1) {
      // Positioning around target in a star pattern
      ...
      this.pedito
        .getMoveControl()
        .setWantedPosition(optimalPos.x, optimalPos.y, optimalPos.z, 2.0D);
      ...
      if (this.timer <= 0) {
        ...
        this.dashDirection = targetPos
          .subtract(peditoPos)
          .normalize()
          .scale(1.0); // 20 m/s = 1 block/tick
      ...
      if (this.pedito.level() instanceof ServerLevel serverLevel) {
        ...
        serverLevel.sendParticles(
          ...
          this.pedito.getX(),
          this.pedito.getY() + this.pedito.getBbHeight() / 2.0,
          this.pedito.getZ(),
          10,
          0.5,
          0.5,
        ...
        List<LivingEntity> hitEntities = this.pedito
          .level()
          .getEntitiesOfClass(
            LivingEntity.class,
            boundingBox,
            e -> e != this.pedito && !e.isAlliedTo(this.pedito)
          );
        for (LivingEntity e : hitEntities) {
          if (
            this.pedito.isTamedByOwner() &&
            (e == this.pedito.getOwnerCustom() ||
              (e instanceof PeditoEntity otherPedito &&
          ...
          e.setDeltaMovement(
            e.getDeltaMovement().add(this.dashDirection.scale(0.5))
          );
        }
      }
      ...
      if (this.timer <= 0 || this.pedito.horizontalCollision) {
        ...
        this.nextAttackTick =
          this.pedito.tickCount + (this.pedito.hasCopperSynergy() ? 88 : 100); // 5 seconds
      }
    }
  }
