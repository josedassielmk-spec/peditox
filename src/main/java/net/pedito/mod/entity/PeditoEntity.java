...
public class PeditoEntity extends Animal {
	...
	private static final double[][] TELEPORT_OFFSETS = {
			...
			{-0.5, 0.6, -2.4}, {3.4, -0.4, 2.3}, {-3.0, 0.6, 0.9}, {0.3, -1.0, 4.4},
			{2.7, -0.7, -2.5}, {5.6, -0.9, -4.5}, {-0.9, -0.5, 3.0}, {-4.0, -0.8, -0.1},
			{-5.3, 0.3, 2.0}, {3.1, 0.9, 0.9}, {4.4, 0.4, -2.2}, {2.3, 0.7, 1.1},
			{0.9, 0.8, -0.5}, {3.9, -1.2, 5.2}, {-0.3, 0.4, 1.9}, {-5.1, 0.8, 2.4}
	};
	private static final int FART_ONLY_WEIGHT = 47;
	private static final int TOTAL_WEIGHT = FART_ONLY_WEIGHT + TELEPORT_OFFSETS.length;
	...
	protected void readAdditionalSaveData(ValueInput input) {
		...
		int legacyDefault = input.getBooleanOr("PeditoNight", false) ? VARIANT_NIGHT : VARIANT_NORMAL;
	...
	public boolean wantsToAttack(net.minecraft.world.entity.LivingEntity target, net.minecraft.world.entity.LivingEntity owner) {
		if (target instanceof net.minecraft.world.entity.monster.Creeper || target instanceof net.minecraft.world.entity.monster.Ghast) {
			...
			return false;
		} else if (target instanceof net.minecraft.world.entity.player.Player && owner instanceof net.minecraft.world.entity.player.Player && !((net.minecraft.world.entity.player.Player)owner).canHarmPlayer((net.minecraft.world.entity.player.Player)target)) {
			return false;
		} else if (target instanceof net.minecraft.world.entity.TamableAnimal tamable && tamable.isTame() && tamable.getOwner() == owner) {
			return false;
		} else {
			return true;
		}
	...
	private void updateAttributesForTier() {
		...
		if (this.getVariant() == VARIANT_ALPHA) {
			...
			armor = 2.0D;
			attack = 1.5D;
		} else if (currentTier == 2) { // Iron
			health = 16.0D;
			armor = 4.0D;
			attack = 2.0D;
		} else if (currentTier == 3) { // Gold
			health = 20.0D;
			armor = 6.0D;
			attack = 2.5D;
		} else if (currentTier == 4) { // Diamond
			health = 30.0D;
			armor = 10.0D;
			attack = 4.0D;
		} else if (currentTier == 5) { // Netherite
			health = 40.0D;
			armor = 14.0D;
			attack = 6.0D;
		}
	...
	private void updateThreatAssessment() {
		...
		if (--this.threatAssessmentTimer <= 0) {
			...
			if (owner.getLastHurtMob() != null && owner.getLastHurtMob().isAlive() && owner.getLastHurtMob().distanceToSqr(owner) < 400) {
			    danger = true;
			}
			// Check if owner was attacked by someone
			if (owner.getLastHurtByMob() != null && owner.getLastHurtByMob().isAlive() && owner.getLastHurtByMob().distanceToSqr(owner) < 400) {
			    danger = true;
			}
	...
	public void setSittingCustom(boolean sitting) {
		...
		if (sitting && !was && this.level() instanceof ServerLevel serverLevel) {
			...
			serverLevel.sendParticles(ParticleTypes.CLOUD, this.getX(), this.getY() + 0.3, this.getZ(),
	...
	static class PeditoNearestPlayerTargetGoal extends Goal {
		...
		public boolean canUse() {
			...
			Player nearest = this.pedito.level().getNearestPlayer(this.pedito, this.range);
			if (nearest == null || !nearest.isAlive() || nearest.isSpectator() || nearest.isCreative()) {
				return false;
			}
		...
		public boolean canContinueToUse() {
			...
			return !this.pedito.isTamedByOwner() && target != null && target.isAlive();
		}
	...
	public void setTarget(@Nullable LivingEntity target) {
		if (target != null && this.isTamedByOwner() && this.getOwnerCustom() != null) {
	...
	public void tick() {
		super.tick();
		if (!this.level().isClientSide() && this.isTamedByOwner() && !this.isSittingCustom()) {
			Player owner = this.getOwnerCustom();
			if (owner != null && !this.isLeashed() && !this.isPassenger()) {
				if (this.distanceToSqr(owner) > 1600.0D) { // 40 blocks
		...
		if (!this.level().isClientSide()) {
			...
			if (this.getTarget() != null) {
			    ...
			    if (!t.isAlive() || (this.isTamedByOwner() && this.getOwnerCustom() != null && !this.wantsToAttack(t, this.getOwnerCustom()))) {
			        this.setTarget(null);
			    } else if (this.isTamedByOwner() && this.getOwnerCustom() != null && t instanceof net.minecraft.world.entity.player.Player p && p == this.getOwnerCustom()) {
	...
	private void doTimerEvent() {
		...
		if (this.random.nextFloat() < 0.30F) {
			...
			if (roll < FART_ONLY_WEIGHT) {
				playFart(serverLevel, this.getX(), this.getY(), this.getZ(), 1.0F, pitch);
			} else {
				...
				serverLevel.sendParticles(ParticleTypes.PORTAL, targetX, targetY + 0.3, targetZ, 12, 0.3, 0.4, 0.3, 0.02);
	...
	private float getFartPitch() {
		...
		if (this.getVariant() == VARIANT_ALPHA) basePitch = 0.65F; // Grave fart!
		...
		float finalPitch = basePitch + (this.getRandom().nextFloat() - this.getRandom().nextFloat()) * 0.15F;
		return finalPitch < 0.55F ? 0.55F : finalPitch;
	}

	private void playFart(ServerLevel world, double x, double y, double z, float volume, float pitch) {
		if (this.getVariant() == VARIANT_ALPHA) {
			world.sendParticles(ParticleTypes.CLOUD, x, y + 0.5, z, 45, 0.6, 0.3, 0.6, 0.08);
			// 3 main colors: Gold (0xFFD700), Green (0x32CD32), Diamond (0x00FFFF), and a bit of Black (0x050505)
			world.sendParticles(new DustParticleOptions(0xFFD700, 1.2F), x, y + 0.5, z, 25, 0.5, 0.3, 0.5, 0.1);
			world.sendParticles(new DustParticleOptions(0x32CD32, 1.2F), x, y + 0.5, z, 25, 0.5, 0.3, 0.5, 0.1);
			world.sendParticles(new DustParticleOptions(0x00FFFF, 1.2F), x, y + 0.5, z, 25, 0.5, 0.3, 0.5, 0.1);
			world.sendParticles(new DustParticleOptions(0x050505, 1.2F), x, y + 0.5, z, 12, 0.5, 0.3, 0.5, 0.15);
		} else {
			world.sendParticles(ParticleTypes.CLOUD, x, y + 0.3, z, 20, 0.4, 0.2, 0.4, 0.05);
			...
			world.sendParticles(new DustParticleOptions(color, 1.0F), x, y + 0.3, z, 30, 0.3, 0.2, 0.3, 0.1);
		}
		
		world.playSound(null, x, y, z, ModSounds.PEDITO_FART, SoundSource.NEUTRAL, volume * 0.6F, pitch);
	}
	...
	public boolean hasNetheriteLeader() {
		...
		java.util.List<PeditoEntity> allies = this.level().getEntitiesOfClass(PeditoEntity.class, this.getBoundingBox().inflate(40.0D), 
	...
	public boolean hurtServer(ServerLevel level, DamageSource source, float amount) {
		...
		if (source.is(net.minecraft.tags.DamageTypeTags.BYPASSES_ARMOR) && amount <= 1.0F) {
			return false;
		}
		...
		if (this.isTamedByOwner()) {
			...
			if (attacker instanceof PeditoEntity otherPedito && otherPedito.isTamedByOwner()) {
				return false;
			}
			
			// +10% Resistencia Mágica de Líder de Netherite
			if (source.is(net.minecraft.tags.DamageTypeTags.WITCH_RESISTANT_TO) || source.is(net.minecraft.tags.DamageTypeTags.BYPASSES_ARMOR)) {
		...
		if (result) {
			...
			playFart(level, this.getX(), this.getY(), this.getZ(), volume, getFartPitch());
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty,
		...
		if (this.level() instanceof ServerLevel serverLevel) {
			serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
					ModSounds.PEDITO_FART_SPAWN, SoundSource.NEUTRAL, 1.5F, 1.0F);
			serverLevel.sendParticles(ParticleTypes.CLOUD, this.getX(), this.getY() + 0.5, this.getZ(),
					8, 0.2, 0.1, 0.2, 0.01);
		}
		return data;
	}
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (this.isTamedByOwner() && !this.isBaby()) {
			...
			else if (stack.is(ModItems.GAS_CAN_DIAMOND) && currentTier < 4) newTier = 4;
			else if (stack.is(ModItems.GAS_CAN_NETHERITE) && currentTier < 5) newTier = 5;

			if (newTier != -1) {
				if (!this.level().isClientSide()) {
					this.setTier(newTier);
					if (!player.getAbilities().instabuild) stack.shrink(1);
					this.playSound(net.minecraft.sounds.SoundEvents.SMITHING_TABLE_USE, 1.0F, 1.0F);
					if (this.level() instanceof ServerLevel serverLevel) {
						serverLevel.sendParticles(ParticleTypes.HAPPY_VILLAGER, this.getX(), this.getY() + 0.5, this.getZ(),
								15, 0.3, 0.3, 0.3, 0.05);
					}
			...
			if (!this.level().isClientSide()) {
				if (this.level() instanceof ServerLevel serverLevel) {
					serverLevel.sendParticles(ParticleTypes.CLOUD, this.getX(), this.getY() + 0.5, this.getZ(),
							6, 0.2, 0.1, 0.2, 0.01);
					serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
							ModSounds.PEDITO_FART, SoundSource.NEUTRAL, 1.0F, 0.7F);
				}
				...
				if (this.random.nextFloat() < 0.35F) {
					...
					if (this.level() instanceof ServerLevel serverLevel) {
						serverLevel.sendParticles(ParticleTypes.PORTAL, this.getX(), this.getY() + 0.5, this.getZ(),
								10, 0.3, 0.4, 0.3, 0.02);
						serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
								ModSounds.PEDITO_FART, SoundSource.NEUTRAL, 1.0F, 1.2F);
					}
				} else {
				...
				if (!this.level().isClientSide()) {
					...
					if (this.level() instanceof ServerLevel serverLevel) {
						serverLevel.sendParticles(ParticleTypes.HAPPY_VILLAGER, this.getX(), this.getY() + 0.5, this.getZ(),
								15, 0.3, 0.4, 0.3, 0.05);
						serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
								ModSounds.PEDITO_SPRAY, SoundSource.NEUTRAL, 1.0F, 1.0F);
						serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(),
								net.minecraft.sounds.SoundEvents.PLAYER_LEVELUP, SoundSource.NEUTRAL, 0.5F, 1.0F);
					}
				}
				return InteractionResult.SUCCESS;
			}
		...
		if (this.isTamedByOwner() && this.isOwnerCustom(player) && !this.isBaby() && stack.isEmpty()) {
			if (!this.level().isClientSide()) {
	...
	static class ApproachTargetGoal extends Goal {
		...
		public boolean canUse() {
			LivingEntity target = this.pedito.getTarget();
			return !this.pedito.isTamedByOwner() && target != null && target.isAlive()
					&& this.pedito.distanceToSqr(target) > this.withinRadius * this.withinRadius;
		}
		...
		public void tick() {
			LivingEntity target = this.pedito.getTarget();
			if (target != null) {
				this.pedito.getLookControl().setLookAt(target, 10.0F, (float)this.pedito.getMaxHeadXRot());
				if (--this.timeToRecalcPath <= 0) {
					this.timeToRecalcPath = 10;
					this.pedito.getMoveControl().setWantedPosition(target.getX(), target.getY() + target.getBbHeight() / 2.0, target.getZ(), this.speed);
				}
			}
		}
	...
	static class CustomFollowOwnerGoal extends Goal {
		...
		CustomFollowOwnerGoal(PeditoEntity pedito, double speed, float minDistance, float maxDistance) {
			this.pedito = pedito;
			this.speed = speed;
		...
		public boolean canUse() {
			...
			return owner != null && this.pedito.distanceToSqr(owner) > (double) (this.maxDistance * this.maxDistance);
		}
		...
		public boolean canContinueToUse() {
			...
			Player owner = this.pedito.getOwnerCustom();
			return owner != null && this.pedito.distanceToSqr(owner) > (double) (this.minDistance * this.minDistance);
		}
		...
		public void tick() {
			Player owner = this.pedito.getOwnerCustom();
			if (owner != null) {
				this.pedito.getLookControl().setLookAt(owner, 10.0F, (float)this.pedito.getMaxHeadXRot());
	...
	private net.minecraft.core.BlockPos findSafePositionNearby(net.minecraft.core.BlockPos startPos) {
	...
	static class RandomFlyWanderGoal extends Goal {
		...
		public boolean canUse() {
			...
			int chance = this.pedito.isBaby() ? 10 : (!this.pedito.isTamedByOwner() ? 20 : 60);
			...
			while (groundY > this.pedito.level().getMinY() && this.pedito.level().getBlockState(new net.minecraft.core.BlockPos((int)rx, groundY, (int)rz)).isAir()) {
				groundY--;
			}
			...
			net.minecraft.core.BlockPos targetPos = new net.minecraft.core.BlockPos((int)rx, (int)ry, (int)rz);
		...
		public boolean canContinueToUse() {
			...
			return this.moveTime > 0 && this.pedito.distanceToSqr(this.targetX, this.targetY, this.targetZ) > 2.0D;
		}
		...
		public void stop() {
			...
			this.pedito.getMoveControl().setWantedPosition(this.pedito.getX(), this.pedito.getY(), this.pedito.getZ(), 0.0D);
			this.idleTime = this.pedito.isBaby() ? 20 + this.pedito.getRandom().nextInt(20) : (!this.pedito.isTamedByOwner() ? 40 + this.pedito.getRandom().nextInt(40) : 60 + this.pedito.getRandom().nextInt(100));
		}
	}

	public enum SquadRole {
		VANGUARD, ARTILLERY, TACTICAL, ROYAL_GUARD, SOLO
	}
	...
	private void updateSquadRole() {
		if (this.level().isClientSide() || this.tickCount % 20 != 0 || !this.isTamedByOwner()) {
			return;
		}
		...
		java.util.List<PeditoEntity> allies = this.level().getEntitiesOfClass(PeditoEntity.class, owner.getBoundingBox().inflate(64.0D), 
			e -> e.isAlive() && e.isTamedByOwner() && e.getOwnerCustom() == owner);
	...
	private void updateAlphaStatus() {
		...
		if (allies.isEmpty() || allies.stream().mapToInt(Entity::getId).min().orElse(this.getId()) != this.getId()) {
			return;
		}
		...
		if (shouldHaveAlpha) {
			if (currentAlpha == null) {
				...
				if (!candidatePeditos.isEmpty()) {
					PeditoEntity chosen = candidatePeditos.get(this.random.nextInt(candidatePeditos.size()));
					...
					if (chosen.level() instanceof ServerLevel serverLevel) {
						serverLevel.playSound(null, chosen.getX(), chosen.getY(), chosen.getZ(),
								ModSounds.PEDITO_SUMMON, SoundSource.NEUTRAL, 2.0F, 0.6F); // Deep voice!
						serverLevel.sendParticles(ParticleTypes.HAPPY_VILLAGER, chosen.getX(), chosen.getY() + 0.5, chosen.getZ(),
								20, 0.5, 0.5, 0.5, 0.05);
					}
				}
			}
